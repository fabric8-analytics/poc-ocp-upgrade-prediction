...
def main():
    ...
    parser.add_argument(
        "--namespace", help="username/orgname where the repo needs to be forked.", required=True
    )
    parser.add_argument(
        "--no-verify",
        required=True,
    ...
    commit_sources(clusterversion, args.destdir, args.namespace, args.no_verify)
...
def commit_sources(clusterversion, destdir, namespace, noverify):
    ...
    for repopath in path.glob("*"):
        ...
        branch_create = subprocess.run(
            ["git", "checkout", "-b", str(branch_name.stdout.strip().decode("utf-8")[:7])],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            cwd=repopath,
        )
        ...
        push = subprocess.run(
            ["git", "push", "ocp-poc-fork", str(branch_name.stdout.strip().decode("utf-8")[:7])],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            cwd=repopath,
        )
